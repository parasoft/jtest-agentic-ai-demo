...
This skill enables GitHub Copilot CLI to provide Parasoft UTA Test Post-Processing on Java projects with generated tests by UTA, and fix failing tests in a post-creation stage.
...
3. Verify that required script files are present inside `JTEST_UTA_SCRIPT_DIR`:
   - Windows: `build-verify\build-verify.bat` (or `build-verify\build-verify.ps1`);
...
### Step 3: Identify Failing Tests And Tests With Error In Order To Fix Them
...
**If `FINAL_LIST_OF_TESTS_TO_PROCESS` IS NOT SET as an environmental variable, then do:**
   ...
   -  **If there ARE NO** special instructions about what tests should be fixed then fill `FINAL_LIST_OF_TESTS_TO_PROCESS` with failing tests and tests with error from Step 2.
...
3. Iterate over a list of failing tests and tests with error `FINAL_LIST_OF_TESTS_TO_PROCESS` and for each test do:
   ...
   **DO NOT BREAK COMPILATION OF THE CLASS THAT YOU MODIFY**
   3. Focus on the test for which the fix has been applied. **Insert a comment in java-doc on the top of the test method that has been fixed as follow:**
   ```
   Fixed by <agent> with model <model> 
   ``` 
    where `<agent>` -- is agent which provide fix (e.g. copilot, codex and so on), and `<model>` -- is used llm model.
   **Ensure the comment is added for the test with applied fix**. **Double check if comment is added to the correct test!** 
   4. Run a fixed test strictly in the following way:
      ...
      - **Otherwise** (no `JTEST_UTA_SCRIPT_DIR`):
         - print the following error message:
            `ERROR: The build-verify scripts are not properly set at JTEST_UTA_SCRIPT_DIR=[JTEST_UTA_SCRIPT_DIR].`
   5. Find and check if the fixed test pass. **If test pass add it to the list of fixed tests `FIXED_TESTS`.** 
   6. If fixed test still fail make `JTEST_FIX_ATTEMPTS` more attempts to fix it: repeat steps from 3.3.1 to 3.3.5 `JTEST_FIX_ATTEMPTS` times for the test that fail.
   7. If test with fix still fail after `JTEST_FIX_ATTEMPTS` provided attempts revert the fix.
   8. If the value of `JTEST_UTA_NO_OF_MAX_FIXES` is more than 0 and the total number of attempted fixes has reached or exceeded `JTEST_UTA_NO_OF_MAX_FIXES`, then stop fixing tests and proceed to Step 4. Provide a warning message in the console:
...
### Step 4: Handle Remaining Failing Tests
